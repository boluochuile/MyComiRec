...
def evaluate_full(sess, topN, hidden_units, test_data, model, model_path, batch_size, item_cate_map, save=True, coef=None):

    item_embs = model.output_item(sess)

    try:
        cpu_index = faiss.IndexFlatL2(hidden_units)
